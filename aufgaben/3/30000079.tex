...
\pfeil{(q1)}{(e)}
\pfeil{(q2)}{(e)}
\node at ($0.5*(q2)+0.5*(e)$) [below right] {$\Sigma$};
\node at ($0.5*(q0)+0.5*(e)$) [below left] {$\texttt{1}$};
\node at ($0.5*(q1)+0.5*(e)$) [right] {$\texttt{1}$};
\node at ($0.5*(q0)+0.5*(q1)$) [above] {$\texttt{1}$};
\node at ($0.5*(q1)+0.5*(q2)$) [above] {$\texttt{0}$};
\draw[->,shorten >= 0.5cm,shorten <= 0.5cm] (q0) to[out=45,in=135,distance=2cm] (q0);
\node at ($(q0)+(0,1.2)$) [above] {\texttt{0}};
\draw[->,shorten >= 0.5cm,shorten <= 0.5cm] (e) to[out=-45,in=-135,distance=2cm] (e);
