...
\def\buchstaben#1{
	...
	\zustand{(q1)}
	\akzeptierzustand{(q2)}{#1}
	\zustand{(q5)}
	\akzeptierzustand{(q6)}{#1}
	\zustand{(q7)}
	\akzeptierzustand{(q8)}{#1}
	\zustand{(q9)}
	\akzeptierzustand{(q10)}{#1}
	\zustand{(q11)}
	\akzeptierzustand{(q12)}{#1}
}
\def\verkettung{
	\begin{scope}
		\color{red}
