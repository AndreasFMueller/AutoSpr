...
Auf dem Input $w$:
\begin{compactenum}
\item Fahre von links nach rechts "uber das Band und streiche jedes zweite $\texttt{0}$.
...
k"onnen. Es ist also $\Gamma = \{\texttt{0},{\tt x},\blank\}$.
Ausserdem brauchen wir
Zust"ande, mit denen man die Parit"at der Anzahl $\texttt{0}$ bestimmen kann, und
...
ein {\tt x} keine weiteren $\texttt{0}$ mehr gefunden werden k"onnen, kann durch
\[
\entrymodifiers={++[o][F]}
\xymatrix{
\ar[r]^{\texttt{0}\to{\tt x},\text{R}}
...
Zun"achst stellen wir sicher, dass der Input tats"achlich mit \texttt{0}
beginnt:
\[
\entrymodifiers={++[o][F]}
\xymatrix{
...
\]
Jetzt h"angen wir das Segment an, welches pr"uft, ob es noch genau ein $\texttt{0}$
auf dem Band hat:
\[
\entrymodifiers={++[o][F]}
\xymatrix{
...
von \texttt{0} gefunden, gem"ass Schritt 3 des Algorithmus muss also verworfen
werden:
\[
\entrymodifiers={++[o][F]}
\xymatrix{
...
ein ``Band'' mit Bandalphabet $\{\texttt{0},1,\blank\}$, erst zus"atzliche
...
\]
muss jetzt "ubersetzt werden in einen "Ubergang von $q_\text{accept}$ in einen
...
Menge der Stackautomaten abz"ahlbar unendlich.
