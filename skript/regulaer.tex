...
Ausser "uber seine Zust"ande verf"ugt ein Automat "uber keinen weiteren
Speicher.
...
in einer endlichen Zahl verschiedener Zust"ande sein kann.
Der "Ubergang zwischen den Zust"anden geschieht deterministisch beim
...
versucht, "andert sich daran nichts.
Erst wenn er eine Fahrkarte einschiebt,
wird das Drehkreuz entriegelt.
Jetzt n"utzt es nichts, noch weitere
...
sind.
Dazu m"ussen die einzelnen Zust"ande ausgezeichnet werden.
...
Die Zust"ande bilden eine Menge.
Ausserdem m"ussen wir festlegen, in welchem Zustand
...
Die Zust"ande von $Q$ werden durch Kreise dargestellt.
Die "Ubergangsfunktion wird durch mit Buchstaben des Alphabets $\Sigma$
angeschriebene Kreise symbolisiert.
Akzeptierzust"ande werden durch doppelte Kreise markiert.
...
Danach d"urfen beliebige
Ziffern folgen, aber kein {\tt -}.
Sobald ein {\tt -} eingegeben
wird, muss der Automat den Input als Illegal erkennen.
Offenbar gibt es also folgende F"alle: Zahl mit Minus, Zahl ohne Minus,
f"uhrende Null oder andere Illegalit"aten.
Zusammen mit dem Startzustand sollten wir also mit vier Zust"anden auskommen.
...
der Anforderungen, dass er keine f"uhrende $0$ akzeptiert.
Allerdings
...
darstellen.
Die Zeilen werden mit den Zust"anden angeschrieben,
die Spalten mit den Zeichen des Alphabetes.
Akzeptierzust"ande werden
...
\begin{beispiel}[\bf Gerade Zahlen]
Die Zust"ande $q_0$ und $q_1$ stehen f"ur gerade und ungerade Zahlen:
...
$L$ besteht aus den W"ortern, die  eine gerade Anzahl von {\tt a}s enthalten.
Diese Sprache ist regul"ar, weil sie von folgendem Automaten akzeptiert wird:
\[
\entrymodifiers={++[o][F]}
\xymatrix{
*+\txt{} \ar[r]
...
die Zust"ande zu charakterisieren.
Befindet sich ein Automat im Zustand $q$, dann kann durch zus"atzlichen
Input $w_e$ ein akzeptables Wort entstehen.
Sei also $L(q)$ die Menge der W"orter, mit deren Hilfe man vom Zustand $q$
...
Dann f"uhrt der Input $w$ den Anfangszustand $L$ schrittweise "uber in
\[
...
\{L(w)\;|\; w\in\Sigma^*\}
\]
endlich ist, ist $L$ regul"ar.
Falls sie unendlich ist, kann $L$ nicht regul"ar sein.
\end{satz}

Als Anwendung dieses Satzes zeigen wir, dass die Sprache
$\{ \texttt{0}^n\texttt{1}^n \;|\; n\in\mathbb N\}$ "uber
$\Sigma=\{\texttt{0},\texttt{1}\}$
nicht regul"ar ist.
Wir m"ussen also die Mengen $L(w)$ f"ur alle denkbaren W"orter $w\in\Sigma^*$
ermitteln.
...
Zun"achst m"usste man beide Automaten in die Standardform bringen.
Wenn die Standardformen "ubereinstimmen, akzeptieren sie die gleiche Sprache.
...
Dann gibt es eine surjektive Abbildung der Zust"ande $f\colon Q\to Q'$, so dass
\begin{compactenum}
\item $f(q_0)=q_0'=L$
...
F"uhren zwei W"orter $w_1$ und $w_2$ den DEA $A$ in den gleichen Zustand $q$,
werden von dort aus die gleichen W"orter akzeptiert, d.\,h.~$L(w_1)=L(w_2)$.
Die Abbildung
\[
...
kann.
Das sind nat"urlich genau diejenigen, die das gleiche $L(q)$
haben.
...
Der Satz~\ref{satz_dea_aus_sprache} zeigt, wie man zu einer regul"aren Sprache
einen endlichen Automaten finden kann.
Als Nebeneffekt kann man daraus oft auch ableiten, ob eine Sprache regul"ar
ist.
Wenn allerdings die Zahl der Zust"ande sehr gross ist, kann man beim
Auflisten der Zust"ande nicht unterscheiden, ob man einfach noch etwas
mehr Geduld haben muss, bis man alle Zust"ande gefunden hat, oder ob
es tats"achlich unendlich viele verschiedene Mengen unter den $L(w)$
gibt.
Wir brauchen daher eine Methode, mit der man die Frage nach der
Regularit"at entscheiden k"onnen, ohne auf eine Auflistung aller
n"otigen Zust"ande angewiesen zu sein.

Eine regul"are Sprache wird von einem DEA akzeptiert.
Dadurch wird die Struktur der W"orter eingeschr"ankt.
...
Dann k"onnen wir die Schleife $y$
...
Endzustand f"uhrt.
Die W"orter $xy^kz$ werden also alle auch von dem Automaten akzeptiert.
...
nachzuweisen, dass sie nicht regul"ar ist.
Dazu nimmt man an, dass die Sprache regul"ar ist, f"ur sie also die Aussage
...
durch, welche wir bereits als nicht regul"ar erkannt haben.
Wir nehmen jetzt also an, dass $L$ regul"ar sei.
Dann gibt es nach dem Pumping Lemma eine Zahl $N$, so dass W"orter mit
mindestens dieser L"ange interessante Eigenschaften haben.
...
haben, also aus lauter Nullen bestehen m"ussen.
Der Teil $y$ muss mindestens eine Null enthalten.
Wenn man jetzt $xy^kz$ bildet, vermehrt man die Zahl der Nullen,
nicht aber die Zahl der Einsen, man erh"alt also ein Wort, welches
mehr Nullen als Einsen enth"alt.
...
$xy^kz\not\in L$ f"ur $k>1$.
Dieser Widerspruch zeigt, dass die urspr"ungliche
Annahme, $L$ sei regul"ar gewesen, nicht haltbar ist.
Also ist die Sprache nicht regul"ar.
\begin{figure}
\begin{center}
\begin{tabular}{l}
...
$\mathstrut^t$-Operation nicht "andern: $w^t=w$.
...
\item Wir pr"ufen dies nach f"ur $k=2$ (Abbildung~\ref{regular:pl-palindrome}).
\begin{figure}
\centering
...
Ein nicht deterministischer Automat l"asst dagegen mehrere M"oglichkeiten zu.
Das ``Resultat''
...
$\varepsilon$-"Uberg"ange vorkommen.
Die zweite Bedingung (\ref{deanea2}) bedeutet, dass von jedem Zustand zu
jedem Zeichen genau ein neuer Zustand m"oglich ist.
...
visualisiert werden.
Die Vertizes sind die Zust"ande $V=Q$.
...
Wir k"onnten diesen Zustand einfach
...
denn vom Zustand $m$
aus gibt es keinen mit \texttt{0} angeschrieben "Ubergang.
\end{beispiel}
...
"Uberg"ange f"ur das Zeichen {\tt a} m"oglich sind.
Der Automat bleibt
...
k"onnen.
Es besteht ein Unterschied, ob der "Ubergang infolge eines
...
von $q$ aus erreichbaren Zust"ande von $Q$ an.
F"ur den Input $a_1a_2$ kann
man von jedem Zustand in $\delta(q,a_1)$ aus weiter Zust"ande mit einem
"Uberang mit Inputzeichen $a_2$ erreichen.
Die Menge der erreichbaren Zust"ande ist
\begin{equation}
...
mit $q_k\in\delta(q_{k-1},a_k)$ f"ur alle $k$.
Ausserdem  ist nat"urlich $q_k\in\delta(q,a_1\dots a_k)$ f"ur alle $k$.

Eine Menge $M$ von Zust"anden stellt man sich am besten vor, indem man die
darin enthaltenen Zust"ande farbig markiert.
In der Menge $\delta(M,a)$ befinden sich alle Zust"ande, die von Zust"anden
...
erreichen.
Die Menge der Zust"ande, die von $q\in Q$ aus durch
...
Die m"oglichen Zust"ande sind jeweils
...
$q_2$ als Folgezust"ande m"oglich.
Die markierte m"ogliche Menge von Zust"anden nach Verarbeitung von {\tt ba}
ist daher $\{q_1,q_2\}$.
...
verbreitet war.
Sp"ater hat Henry Spencer die Regex-Library neu
...
Sprachen.
Dazu gen"ugt es zu zeigen, dass sich jeder NEA in einen DEA
...
f"ur gewisse $q\in Q$ und $a\in\Sigma$.
...
der NEA sein k"onnte.
Seine Zust"ande sind als Mengen von m"oglichen Zust"anden des NEA.
Als Zustandsmenge des DEA wird man also $P(Q)$ 
erwarten.
Wie die "ubrigen Elemente des DEA konstruiert werden m"ussen,
...
Graphen von $A$, der zu einem Akzeptierzustand f"uhrt.
Also enth"alt
...
wird.
Da diese Menge einen Akzeptierzustand enth"alt, ist sie ein
...
Aus dem Satz ist bekannt, dass die Zust"ande die Potenzmengen von $Q$ sind.
Diese entsprechen den dreistelligen Bin"arzahlen, wobei jede
Stelle angibt, ob das zugeh"orige $q_i$ in der Menge dabei ist.
Wir
schreiben also
\begin{align*}
...
Etwas "ubersichtlicher geschrieben lautet er
\[
\entrymodifiers={++[o][F]}
\xymatrix{
*+\txt{}\ar[r]
...
Anfangszustand von $\{q_0\}$ auf $E(q_0)$ vergr"ossert haben.
Dadurch erfassen wir alle Pfade durch den Automaten, die mit einem oder
mehreren $\varepsilon$-"Uberg"angen beginnen.
Falls $E(q_0)\cap F\ne \emptyset$
...
Jetzt m"ussen wir nur die beiden
im Satz \ref{satz_neadea} erw"ahnten Modifikationen durchf"uhren.

Urspr"unglich war der Startzustand $q_{001}$.
...
wird.
Dies bedeutet im vorliegenden Fall, dass bei jedem "Ubergang,
...
kann, l"asst sich dieser Automat nicht mehr weiter reduzieren.
Etwas "ubersichtlicher geschrieben lautet er
\[
\entrymodifiers={++[o][F]}
\xymatrix{
...
Resultat.
Wenn man die endliche vielen Zust"ande mit den Zahlen
$0,\dots,|Q|-1$ identifiziert, kann man die Teilmengen von $Q$ mit
den $|Q|$-stelligen Bin"arzahlen identifizieren.
Der Teilmenge $M\subset Q$ entsprechen die Bin"arzahlen, die Einsen an
...
\end{align*}
mit $L_1=L(A_1)$ und $L_2=L(A_2)$.
Wir m"ussen jetzt einen Automaten
\[
A = (Q, \Sigma, \delta, q_0, F)
\]
...
Als "Ubergangsfunktion verwenden wir
\[
...
\end{align*}
konstruieren.
Die einzelnen Teilautomaten sind $A_1$ f"ur $L_1$:
\[
\entrymodifiers={++[o][F]}
\xymatrix{
*+\txt{}\ar[r]
...
Dieser neue DEA akzeptiert genau diejenigen W"orter, die $A$ nicht
akzeptiert hat, also $L(A')=\overline{L(A)}$.
Somit ist $\bar L=L(A')$ regul"ar.
...
In der Praxis haben sich regul"are Ausdr"ucke
durchgesetzt, die mit einer einfachen Syntax ebenfalls Mengen von
W"ortern zu spezifizieren erlauben.
...
regul"aren Sprachen herausf"uhren.
Dazu m"ussen wir zu gegebenen Automaten $A_1$ und $A_2$ f"ur zwei Sprachen
$L_1$ und $L_2$ neue Automaten konstruieren, welche
die Sprachen $L_1L_2$ oder $L_1^*$ akzeptieren.
Die Automaten $A_1$ und $A_2$ m"ussen wir dabei als ``Black Box'' betrachten,
...
Ein Automat $A$ f"ur $L=L_1L_2$ kann aus Automaten $A_1$ und $A_2$
...
sind die Akzeptierzust"ande $F_2$ von $A_2$.
Also
\begin{align*}
Q&=Q_1\cup Q_2\\
F&=F_2\\
...
$\varepsilon$-"Ubergang von einem Akzeptierzustand zum Startzustand
...
auch wieder zum Startzustand zur"uckkehren kann.
...
Stil wie f"ur Verkettung und *-Operation finden.
...
die regul"aren Ausdr"ucke definieren.
...
die nicht l"anger als 1 Zeichen sind.
Regul"are Ausdr"ucke f"ur
...
F"ur die regul"aren Operationen verwenden wir die Notation
...
welche Gruppen verkn"upft werden.
Da mit regul"aren Operationen
...
nehmen, und keinen neuen Startzustand hinzuf"ugen.
Bei der
...
\]
\end{beispiel}
Diese Beispiele illustrieren, dass sich aus jedem regul"aren Ausdruck
ein NEA bauen l"asst.
...
Ausdr"ucken.
Dann wandelt man einen DEA zun"achst um in einen
...
Diesen m"ussen wir jetzt auf einen VNEA reduzieren, der nur noch
die beiden Zust"ande $q_{\text{start}}$ und $q_{\text{accept}}$
enth"alt.
Dazu reissen wir nacheinander alle Zust"ande des urspr"unglichen
NEA heraus.
...
herausreissen.
Seien weiter $q_i$ und $q_j$ zwei Zust"ande, die
...
der zus"atzlich zum Ausdruck $r_4$, m"oglich ist.
...
\]
Jetzt entfernen wir nacheinander alle ``inneren'' Zust"ande.
...
F"ur jedes Zeichen sind zwei Zielzust"ande
m"oglich, die in den letzten beiden Spalten eingetragen werden.
Einen
...
Akzeptierzustand.
Die Zust"ande 4 und 5 sind als
...
m"oglicherweise exponentieller Laufzeit.
...
besseren Regex-Matcher beinhalten w"urde.
