...
Dabei sehen wir die Tatsache, dass für
die Darstellung einer reellen Zahl eine unendliche Folge von Ziffern
nötig sein kann, nicht als Hindernis an.
...
Zahl nach der anderen liefern kann.
Die Turingmaschine
...
$e$ und weitere.
Wir werden allerdings sehen, dass die meisten
...
genannt wird.
Unsere Geschichte beginnt kurz nachdem der Nachtportier
die Nachtschicht übernommen hat.
Ein verspäteter Gast meldet sich
...
ein.
Selbstverständlich haben wir noch ein Zimmer für jeden noch so
späten Gast.
Der Nachportier protestiert, es seien doch alle Zimmer
belegt.
``Kein Problem'' sagt der Manager, und greift zum Mikrofon
der Sprechanlage.
Er bittet alle Gäste, aus ihrem aktuellen Zimmer
ins nächste Zimmer umzuziehen.
Dieses Ansinnen zu später Stunden
...
auch auf das Entgegenkommen der anderen Gäste angewiesen.
Auf diese Weise wird das Zimmer mit der Nummer $0$ frei für den neu
angekommenen Gast.

Kurze Zeit später kommt eine verspätete Reisegruppe aus dreissig
Touristen an, die ebenfalls
Einlass verlangt.
Der Nachtportier will nicht nochmals einen schlechten
Eindruck hinterlassen und ruft den Manager.
Dieser greift
wieder zum Mikrofon, und bittet wieder alle Gäste, diesmal aus
ihrem Zimmer mit der Nummer $n$ ins Zimmer mit der Nummer $n+30$
umzuziehen.
So werden die Zimmer mit den Nummern $0$ bis $29$ frei,
...
natürlichen Zahlen.
Der Nachtportier ist schockiert: Unendlich
viele Gäste, ohne Reservation.
Dass man für endlich viele Gäste
...
will die Gesellschaft wegschicken.
Doch der Hotelmanager möchte sich das Geschäft nicht entgehen lassen.
...
Passagiere des Hilbert-Buses.
Nur die Stimmung unter den Stammgästen
hat sich bereits deutlich verschlechtert.

Schliesslich fahren gleich dreissig Hilbertbusse vor.
Ein Hilbert-Jumbo
...
hierfür eine Lösung bereit hat.
Tatsächlich: per Mikrofondurchsage
gibt er wieder alle ungerade Zimmer frei.
Dann bringt er die Neuankömmlinge
...
``Unendlich'' keine Ruhe.
Als nächstes findet sich ein Hilbert-Konvoi
vor dem Hotel ein.
Also einer Folge von Hilbert-Bussen, die mit natürlichen
Zahlen numeriert waren.
Der Nachtportier hat gar nicht erst gewagt,
den Neuankömmlingen abschlägigen Bescheid zu geben, sondern gleich
den Manager gerufen.
Der muss zwar auch einen Moment nachdenken, findet
dann aber eine Lösung.
Zunächst wendet er nochmals den Trick an,
...
eingezeichneten Reihenfolge ab.
Auch dies trägt nicht  unbedingt
...
Geldgier des Managers rächen.
Gegen morgen nämlich fährt ein voller Cantor-Bus vor.
Dieses hochmoderne Verkehrsmittel hat Sitze,
\index{Cantor!-Bus}%
die mit den reellen Zahlen angeschrieben sind.
Natürlich wollen auch
diese Gäste im Hotel ``Unendlich'' untergebracht werden.
Aber so sehr
sich der Manager auch anstrengt, in seinem Hotel kann er diese
Gäste nicht unterbringen.
Daraufhin wird er zwar vom Verwaltungsrat
entlassen, der noch weniger Mathematik versteht, was ihm dank üppiger
Abgangsentschädigung jedoch nicht weiter Sorgen macht.
Unbestätigten
...
Platz genug für eine Kopie der ganzen Menge finden kann.
Oder anders
...
Zahlen konstruiert worden waren.
Mit $\mathbb N$ vergleichbare Mengen
...
gleichmächtig ist wie die natürlichen Zahlen.
$A$ heisst
...
viele Gäste wie $\mathbb N^2$ Paare enthält.
Und alle diese Paare
passen in die Menge $\mathbb N$, also die Zimmer des Hotels ``Unendlich''
hinein.
Somit sind $\mathbb N^2$ und $\mathbb N$ gleich mächtig.
...
Das kartesische Produkt zweier abzählbarer
Mengen ist abzählbar.
\end{satz}
...
Nehmen wir an, $\mathbb R$ wäre abzählbar unendlich.
Dann müssten auch
...
kann.
Um die $k$-te Stelle $x_k$ von $x$ zu konstruieren, betrachten
...
Wir hatten angenommen, dass die
...
Daher haben wir in der Geschichte
...
$i\mapsto a_i\in A$.
Nehmen wir an, es gäbe eine bijektive
...
\]
Die Menge $B$ unterscheidet sich von jeder Menge $A_i$.
Falls $A_i$ das Element $a_i$ enthält, enthält $B$ es nicht und
umgekehrt.
...
Die Menge aller Sprachen über dem
...
Nach Satz \ref{powersetuncountable} ist
...
so weiter.
Weil keine der Mengen $A_i$ unendlich ist, kommt jede
Menge irgendwann dran.
\end{proof}

\begin{satz} Sei $\Sigma$ ein festes Alphabet.
...
Satz \ref{countablefinite} auch die Vereinigung abzählbar.
Ein
deterministischer endlicher Automat ist aber durch die Tabellendarstellung
gegeben.
In der Tabelle sind $|Q|\cdot|\Sigma|$ Felder mit Zuständen
...
Akzeptierzustände sind, das sind $|P(Q)|=2^{|Q|}$ Möglichkeiten.
Es gibt also
\[
...
noch nicht über\-abzählbar.
Der Funktionswert der Übergangsfunktion
$\delta$ ist ja immer eine Teilmenge einer endlichen Menge, die Potenzmenge
einer endlichen Menge ist aber auch endlich.
Es ändert sich am Argument
...
zusätzlicher Beschriftung der Übergangspfeile.
Trotzdem bleibt die
...
mit zusätzlichen Beschriftungen der Pfeile.
Wie in 1.~gibt es nur endlich
...
Turingmaschine als Aufzähler implementieren lässt.
Zähler und
Nenner werden zu Beginn auf das Band geschrieben, der Algorithmus
berechnet dann alle Stellen des Quotienten.
Also sind die rationalen
Zahlen berechenbar.
\end{proof}
...
mit beliebiger Genauigkeit bestimmt werden.
Dazu muss nur eine
...
\]
immer genauere Approximationen berechnet werden.
Alle Operationen
...
\begin{proof}[Beweis]
Die berechenbaren Zahlen sind abzählbar.
Wir können nämlich
...
mit genau zwei Zuständen usw.
Da jede Turingmaschine genau eine
...
entscheidend voranbringen würden.
Einige dieser Probleme wurden
...
Lösungen hat, zum Beispiel $x=3$, $y=4$ und $z=5$.
Seit wenigen
...
ist gemeint? Hilbert hat keine Definition gegeben.
Heute würden
wir wohl fragen, ob ein Algorithmus angegeben werden kann, mit dem
die Frage entschieden werden kann.
Doch das führt uns nur wieder
...
lässt.
Probleme, für die es einen Lösungsalgorithmus gibt, sind
...
Zustand $q_{\text{accept}}$ an.
Falls das Problem keine Lösung hat,
hält die Maschine im Zustand $q_{\text{reject}}$ an.
Solche
...
Wir möchten die Frage
von einem Algorithmus beantworten lassen, ob es in einem Graphen $G$
einen Weg gibt, der alle Knoten trifft.
Dazu müssen wir eine
...
diese laufen lassen, und warten, bis sie die Frage beantwortet.
$L(M)$ besteht aus Beschreibungen $\langle G\rangle$ von Graphen $G$, die
...
Lösung hat.
Wir werden im nächsten Kapitel sehen, dass es viele
Probleme gibt, die keine algorithmische Lösung haben.
Im übernächsten
